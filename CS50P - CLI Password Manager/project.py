...
class PasswordManager:
    ...
    def decrypt_data(self):
        ...
        password = self.master_password or input("Enter master password: ")
    ...
    def create_file(self, filename: str, password: str):
        ...
        self.data = [["Name", "Email", "Username", "Password", "Notes", "Creation", "Modified"]]
    ...
    def add_entry(self, name, email, username, password, notes):
        ...
        self.data.append([name, email, username, password, notes, datetime_stamp, datetime_stamp])
...
def main():
    ...
    while True:
        try:
            ...
            welcome = input("Would you like to: \n [1] Open a new file \n [2] Open an existing file \n Your choice: ").strip()
            ...
            if welcome in acceptance:
                ...
                while True:
                    try:
                        ...
                        filename = input("What would you like to call your new file? (Note: No need to enter file extension in the end.) \n").strip()
            ...
            # Second option from Welcome: opening files
                ...
                while True:
                    ...
                    # Option 1: See all entries in database; will go back to options after printing the database
                    if choice == "1":
                        if len(pm.data) <= 1:
                            print_header("ERROR")
                    ...
                    # Option 2: Adding to database
                        ...
                        while True:
                            try:
                                ...
                                entry = input("Which would you like to open? Choose a number or exit to return to Main Menu. \n")
                                # Check if user exits or convert to int
                                if entry == "exit":
                                    break
                                else: 
                                    entry = int(entry)
                                # Check that input is in range of data
                        ...
                        print("\033[1mNote: All spaces at the beginning and end will automatically be removed\033[0m")
                        print_header("space")
                        ...
                        new_result = pm.add_entry(name, email, username, password, notes)
                        ...
                        while True:
                            try:
                                ...
                                # User selection of row to change
                                entry = input("Which of the entries would you like to modify? Choose number or exit to return to exit fron entry selection \n ")
                                ...
                                if 0 <= selected_entry_value < len(header_row):
                                    entry_confirm = input(f"You selected {header_row[selected_entry_value]}. Confirm? \n [1] Yes \n [2] No \n Your choice: ").strip()
                                    
                                    # If password changing, run password function; else, allow user to update other values
                                    if selected_entry_value == 3 and entry_confirm in acceptance:
                                        new_value = password_choice(pm)
                                        print_header("space")
                                        print_header("UPDATING")
                                        selected_entry[int(selected_entry_value)] = new_value
                                        datetime_stamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    ...
                                    # After user confirm of thier selection, enter value to replace
                                        ...
                                        new_value = input("Please enter your update: ").strip()
                                        print_header("space")
                                        print_header("UPDATING")
                                        selected_entry[int(selected_entry_value)] = new_value
                                        datetime_stamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                ...
                                update_confirm = input("Would you like to update more or return to Main Menu? \n [1] Update \n [2] Main Menu \n Your choice: ").strip()
                                if update_confirm == "1":
                                    continue
                        ...
                        while True:
                            try:
                                ...
                                delete_end = input("Would you like to delete more or return to Main Menu? \n [1] Delete \n [2] Main Menu \n Your choice: ").strip()
                                if delete_end == "1":
                                    continue
                                elif delete_end == "2":
                                    break
...
def generate_password(length):
    ...
    if length <= 8:
        print("Password cannot be less than 8 characters long. This is the standard generation length.")
    ...
    try:
        ...
        with open("data/compromised_list.txt", "r", encoding="utf-8") as file:
            ...
            if password in content:
                print_header("space")
                print("Your password was found in a 1 million list of compromised passwords.It is highly recommend to change it so something stronger. Use the below list of categories as a guide (minimum 2 of each, randomly ordered)")
                strength = len(password) / 2
            elif password not in content:
                print_header("space")
                print("Your password was not found in the compromised list; yet, if it is less than 12 characters, recommend you change it to 12 or more characters in length. Use the below list of categories as a guide (minimum 2 of each, randomly ordered)")
                strength += len(password) / 2
...
def delete_entry(pm: PasswordManager):
    try:
        ...
        print_message("m_delete_reminder") 
        print_header("space")
        delete_confirm = input(f"Confirm delete? \n [1] Yes \n [2] No \n Your choice: ").strip()
...
def print_message(message):
    ...
    if message == "m_file_exists":
        ...
        print("Note: All characters, numbers and symbols are accepted. However, passwords cannot start or end with spaces - they will be removed automatically.")
        ...
        print("Invalid choice. Please choose the number of the required file/option. Restarting...")
        ...
        print("If you would like to exit from the program, press Ctrl+C. If a database is open, it will close it and encrypt it.")
    elif message == "m_delete_reminder":
        print("Once data is deleted, it cannot be restored. Be careful with the your selections.")
    ...
    while True:
        try:
            ...
            if pm.filepath is None:
                new_choice = input("Would you like to: \n [1] Enter your own master password \n [2] Generate a password \n Your choice: ").strip()
            else:
                new_choice = input("Would like to: \n [1] Enter your own password \n [2] Generate a password \n Your choice:  ")
            # User input password
            if new_choice in acceptance:
                ...
                if not password and pm.filepath is None:
                    print_header("ERROR")
                    print("\033[1mThis is a master password and should be strong. It cannot be empty.\033[0m \n")
                    print_header("RESTARTING PASSWORD CREATOR")
                    continue
                elif not password:
                    no_password = input("No password was entered.\n [1] Agree \n [2] Redo \n Your choice: ")
                ...
                if pm.filepath is None:
                    ...
                    if check != "Strong":
                        print("\033[1mThis is a master password and should be strong. It is recommended for you to change your password and either remember it or write it down somewhere safe. Use the categories above as a guide (2 of each in random order) or use the password generator option. \033[0m \n")
                else:              
                    strength_check = input("Strength check your password? \n [1] Yes \n [2] No \n Your choice: ").strip()
                    if strength_check in acceptance:
                        ...
                        print_message("m_invalid")
                        continue
                password_accept = input("Would you like to: \n [1] Keep the password \n [2] Change the password \n Your choice: ").strip()
            ...
            # Generating password option
                ...
                password_length = input("How long would you like your password to be? (Recommended: 12) \n Your choice: ").strip()
                
                if password_length.isdigit() and pm.filepath is None:
                    password = generate_password(int(password_length))
                    print(f"The generated password is: {password} \n")
                    print("Please save this password in a safe place. If you lose this password, you will lose access to the file and all its contents.")
...
def user_exit(pm: PasswordManager):
    ...
    if exit_confirm == "1" or not exit_confirm:
        if pm.filepath is not None:
            save_confirm = input("Would you like to save your data first? \n [1] Yes \n [2] No \n Your choice: ")
